...
For the first deployment, run the API with a `ConnectionStrings:WorkslipDb` value pointing to Azure SQL and execute `SqlMigrationRunner.ApplyAsync` from an admin-only startup task or one-off deployment command.
